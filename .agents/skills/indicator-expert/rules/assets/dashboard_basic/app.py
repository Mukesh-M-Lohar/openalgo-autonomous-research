...
def update_chart(n_clicks, symbol, exchange, interval, overlays, subplots):
    ...
    if "ema20" in overlays:
        fig.add_trace(
            go.Scatter(
                x=x_labels, y=ta.ema(close, 20), name="EMA(20)", line=dict(color="cyan", width=1.5)
            ),
            row=1,
            col=1,
        )
    ...
    if "bbands" in overlays:
        ...
        fig.add_trace(
            go.Scatter(
                x=x_labels, y=upper, name="BB Upper", line=dict(color="gray", dash="dash", width=1)
            ),
            row=1,
            col=1,
        )
    ...
    if "supertrend" in overlays:
        ...
        fig.add_trace(
            go.Scatter(x=x_labels, y=st_up, name="ST Up", line=dict(color="lime", width=2)),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(x=x_labels, y=st_down, name="ST Down", line=dict(color="red", width=2)),
            row=1,
            col=1,
        )
    ...
    for i, sp in enumerate(subplots, start=2):
        if sp == "rsi":
            ...
            fig.add_trace(
                go.Scatter(x=x_labels, y=m, name="MACD", line=dict(color="cyan", width=1)),
                row=i,
                col=1,
            )
            fig.add_trace(
                go.Scatter(x=x_labels, y=s, name="Signal", line=dict(color="orange", width=1)),
                row=i,
                col=1,
            )
            colors = ["green" if v >= 0 else "red" for v in h]
            fig.add_trace(
                go.Bar(x=x_labels, y=h, name="Hist", marker_color=colors, opacity=0.6), row=i, col=1
            )
        elif sp == "volume":
            vc = ["green" if c >= o else "red" for c, o in zip(close, df["open"])]
            fig.add_trace(
                go.Bar(x=x_labels, y=df["volume"], name="Volume", marker_color=vc, opacity=0.5),
                row=i,
                col=1,
            )
        elif sp == "stochastic":
            ...
            fig.add_trace(
                go.Scatter(x=x_labels, y=k, name="%K", line=dict(color="cyan", width=1)),
                row=i,
                col=1,
            )
            fig.add_trace(
                go.Scatter(x=x_labels, y=d, name="%D", line=dict(color="orange", width=1)),
                row=i,
                col=1,
            )
            ...
            fig.add_trace(
                go.Scatter(x=x_labels, y=dp, name="+DI", line=dict(color="lime", width=1)),
                row=i,
                col=1,
            )
            fig.add_trace(
                go.Scatter(x=x_labels, y=dm, name="-DI", line=dict(color="red", width=1)),
                row=i,
                col=1,
            )
            fig.add_trace(
                go.Scatter(x=x_labels, y=adx, name="ADX", line=dict(color="yellow", width=1.5)),
                row=i,
                col=1,
            )
