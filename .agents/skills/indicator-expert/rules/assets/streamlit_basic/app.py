...
refresh_sec = st.sidebar.slider("Refresh interval (sec)", 5, 60, 30, disabled=not auto_refresh)
...
try:
    ...
    c5.metric("EMA(20)", f"{ema20_val:,.2f}", f"{'Above' if ltp > ema20_val else 'Below'}")
except Exception:
    pass
...
if "EMA(20)" in overlays:
    fig.add_trace(
        go.Scatter(
            x=x_labels, y=ta.ema(close, 20), name="EMA(20)", line=dict(color="cyan", width=1)
        ),
        row=1,
        col=1,
    )
if "EMA(50)" in overlays:
    fig.add_trace(
        go.Scatter(
            x=x_labels, y=ta.ema(close, 50), name="EMA(50)", line=dict(color="orange", width=1)
        ),
        row=1,
        col=1,
    )
if "Bollinger Bands" in overlays:
    ...
    fig.add_trace(
        go.Scatter(x=x_labels, y=upper, name="BB Upper", line=dict(color="gray", dash="dash")),
        row=1,
        col=1,
    )
    fig.add_trace(
...
for i, sp in enumerate(subplots, start=2):
    if sp == "RSI":
        rsi = ta.rsi(close, 14)
        fig.add_trace(
            go.Scatter(x=x_labels, y=rsi, name="RSI(14)", line=dict(color="yellow", width=1)),
            row=i,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(x=x_labels, y=m, name="MACD", line=dict(color="cyan")), row=i, col=1
        )
        fig.add_trace(
            go.Scatter(x=x_labels, y=s, name="Signal", line=dict(color="orange")), row=i, col=1
        )
        colors_h = ["green" if v >= 0 else "red" for v in h]
        fig.add_trace(
            go.Bar(x=x_labels, y=h, name="Hist", marker_color=colors_h, opacity=0.6), row=i, col=1
        )
    elif sp == "Volume":
        vc = ["green" if c >= o else "red" for c, o in zip(close, df["open"])]
        fig.add_trace(
            go.Bar(x=x_labels, y=volume, name="Volume", marker_color=vc, opacity=0.5), row=i, col=1
        )
    elif sp == "Stochastic":
        k, d = ta.stochastic(high, low, close, k_period=14, smooth_k=3, d_period=3)
        fig.add_trace(
            go.Scatter(x=x_labels, y=k, name="%K", line=dict(color="cyan", width=1)), row=i, col=1
        )
        fig.add_trace(
            go.Scatter(x=x_labels, y=d, name="%D", line=dict(color="orange", width=1)), row=i, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(x=x_labels, y=adx_val, name="ADX", line=dict(color="white", width=1.5)),
            row=i,
            col=1,
        )
        fig.add_trace(
            go.Scatter(x=x_labels, y=plus_di, name="+DI", line=dict(color="green", width=1)),
            row=i,
            col=1,
        )
        fig.add_trace(
            go.Scatter(x=x_labels, y=minus_di, name="-DI", line=dict(color="red", width=1)),
            row=i,
            col=1,
        )
    elif sp == "OBV":
        fig.add_trace(
            go.Scatter(
                x=x_labels, y=ta.obv(close, volume), name="OBV", line=dict(color="magenta", width=1)
            ),
            row=i,
            col=1,
        )
