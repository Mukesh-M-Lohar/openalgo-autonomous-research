...
refresh_sec = st.sidebar.slider("Refresh interval (sec)", 10, 120, 60, disabled=not auto_refresh)
...
for idx, (tf_name, tf_cfg) in enumerate(TIMEFRAMES.items()):
    with all_cols[idx]:
        ...
        fig.add_trace(
            go.Scatter(
                x=x, y=ema_20, name="EMA(20)", line=dict(color="cyan", width=1), showlegend=False
            )
        )
        fig.add_trace(
            go.Scatter(
                x=x, y=ema_50, name="EMA(50)", line=dict(color="orange", width=1), showlegend=False
            )
        )
