...
print("\n" + "=" * 60)
print(
    f"  Buy & Hold: {WEIGHTS['NIFTYBEES'] * 100:.0f}% NIFTYBEES + {WEIGHTS['GOLDBEES'] * 100:.0f}% GOLDBEES"
)
print("=" * 60)
print(pf.stats())
...
print("* This is a PASSIVE strategy - buy once and hold, no rebalancing needed")
print("* Gold (GOLDBEES) provides diversification when equity markets fall")
...
fig = make_subplots(
    rows=2, cols=1, shared_xaxes=True, row_heights=[0.65, 0.35], vertical_spacing=0.07
)
...
fig.add_trace(
    go.Scatter(
        ...
        y=cum_nifty * 100,
        name="NIFTY 50",
        line=dict(color="#ff6688", width=1.5, dash="dash"),
    ),
    row=1,
    col=1,
)
