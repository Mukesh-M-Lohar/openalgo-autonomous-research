...
print(
    ...
    ].to_string(index=False, float_format=lambda x: f"{x:.2%}" if abs(x) < 10 else f"{x:.2f}")
)
...
print("  -> WFE > 50% is acceptable, > 70% is good")
...
fig.update_layout(
    template="plotly_dark", title="Walk-Forward Analysis Results", height=600, showlegend=True
)
