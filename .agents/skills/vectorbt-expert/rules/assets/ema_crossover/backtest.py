...
print(
    f"* Total Return: Strategy returned {pf.total_return() * 100:.2f}% vs "
...
print("  -> Measures risk-adjusted return. >1 acceptable, >2 excellent.")
print(
    f"* Sortino Ratio: {pf.sortino_ratio():.2f} "
    f"({'Good' if pf.sortino_ratio() > 1.5 else 'Moderate'})"
)
print("  -> Like Sharpe but only penalizes downside volatility.")
print(f"* Max Drawdown: {pf.max_drawdown() * 100:.2f}%")
print("  -> Worst peak-to-trough decline. Can you stomach this loss?")
print(f"* Win Rate: {pf.trades.win_rate() * 100:.1f}%")
print("  -> Percentage of winning trades. EMA crossover typically 35-45%.")
print(
...
print("  -> Gross profit / gross loss. >1.5 is good, >2 is excellent.")
print(f"* Total Trades: {pf.trades.count()}")
print(
    f"  -> {'Sufficient for statistical significance' if pf.trades.count() >= 30 else 'Too few trades - results unreliable'}"
)
