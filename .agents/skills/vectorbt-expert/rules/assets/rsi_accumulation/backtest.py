...
print("  RSI 50-68: 5% | RSI 30-50: 10% | RSI <30: 20%")
...
comparison = pd.DataFrame(
    ...
    index=["Total Return", "CAGR", "Sharpe Ratio", "Sortino Ratio", "Max Drawdown", "Final Value"],
)
...
print(
    f"  -> On Rs {INIT_CASH:,} capital, the worst temporary loss = Rs {abs(pf.max_drawdown()) * INIT_CASH:,.0f}"
)
print("* This is a SYSTEMATIC ACCUMULATION strategy")
print("  -> Buys more when market is fearful (low RSI) and less when greedy (high RSI)")
print("  -> Exits everything when RSI signals extreme overbought conditions")
...
fig.add_trace(
    go.Scatter(
        x=niftybees_bh_equity.index,
        y=niftybees_bh_equity.values,
        name="NIFTYBEES B&H",
...
fig.add_trace(
    go.Scatter(
        x=fd_equity.index,
        y=fd_equity.values,
        name=f"HDFC FD {FD_CAGR * 100:.2f}%",
        line=dict(color="#888888", width=1.5, dash="dashdot"),
    ),
    row=1,
    col=1,
)
...
fig.add_trace(
    go.Scatter(
        x=drawdown_daily.index,
        y=drawdown_daily.values,
        name="Drawdown",
        fill="tozeroy",
...
fig.add_hline(
    ...
    line_dash="dash",
    line_color="#00d4aa",
    annotation_text=f"Buy < {RSI_BUY_THRESHOLD}",
    row=3,
    col=1,
)
fig.add_hline(
    y=RSI_EXIT_THRESHOLD,
    line_dash="dash",
    line_color="#ff4444",
    annotation_text=f"Exit > {RSI_EXIT_THRESHOLD}",
    row=3,
    col=1,
)
