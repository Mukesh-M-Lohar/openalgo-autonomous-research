        ...
        for idx, row in trade_candles.iterrows():
            ...
            if position == 0 and trades_count < max_trades_per_day:
                if can_enter:
                    ...
                    triggered_long = (high_val >= long_trigger) and (
                        trade_direction in ["both", "long"]
                    )
                    triggered_short = (low_val <= short_trigger) and (
                        trade_direction in ["both", "short"]
                    )
...
if __name__ == "__main__":
    ...
    print(
        f"NIFTY Train ADR: {n_metrics['train']['adr']:.4f}%, Sharpe: {n_metrics['train']['sharpe']:.2f}, MaxDD: {n_metrics['train']['max_dd']:.2f}%, WinRate: {n_metrics['train']['win_rate']:.1f}%, PosDays: {n_metrics['train']['positive_days_pct']:.1f}%, Trades: {n_metrics['train']['trades']}"
    )
    print(
        f"NIFTY Val ADR: {n_metrics['val']['adr']:.4f}%, Sharpe: {n_metrics['val']['sharpe']:.2f}, MaxDD: {n_metrics['val']['max_dd']:.2f}%, WinRate: {n_metrics['val']['win_rate']:.1f}%, PosDays: {n_metrics['val']['positive_days_pct']:.1f}%, Trades: {n_metrics['val']['trades']}"
    )
    print(
        f"BANKNIFTY Train ADR: {b_metrics['train']['adr']:.4f}%, Sharpe: {b_metrics['train']['sharpe']:.2f}, MaxDD: {b_metrics['train']['max_dd']:.2f}%, WinRate: {b_metrics['train']['win_rate']:.1f}%, PosDays: {b_metrics['train']['positive_days_pct']:.1f}%, Trades: {b_metrics['train']['trades']}"
    )
    print(
        f"BANKNIFTY Val ADR: {b_metrics['val']['adr']:.4f}%, Sharpe: {b_metrics['val']['sharpe']:.2f}, MaxDD: {b_metrics['val']['max_dd']:.2f}%, WinRate: {b_metrics['val']['win_rate']:.1f}%, PosDays: {b_metrics['val']['positive_days_pct']:.1f}%, Trades: {b_metrics['val']['trades']}"
    )
    ...
    for idx, exp in enumerate(experiments):
        ...
        print(
            f"   - Train Period: ADR = {n_m['train']['adr']:.4f}%, Sharpe = {n_m['train']['sharpe']:.2f}, MaxDD = {n_m['train']['max_dd']:.2f}%, WinRate = {n_m['train']['win_rate']:.1f}%, PosDays = {n_m['train']['positive_days_pct']:.1f}%, Trades = {n_m['train']['trades']}"
        )
        print(
            f"   - Validation Period: ADR = {n_m['val']['adr']:.4f}%, Sharpe = {n_m['val']['sharpe']:.2f}, MaxDD = {n_m['val']['max_dd']:.2f}%, WinRate = {n_m['val']['win_rate']:.1f}%, PosDays = {n_m['val']['positive_days_pct']:.1f}%, Trades = {n_m['val']['trades']}"
        )
        print(
            f"   - Out-of-Sample Period: ADR = {n_m['oos']['adr']:.4f}%, Sharpe = {n_m['oos']['sharpe']:.2f}, MaxDD = {n_m['oos']['max_dd']:.2f}%, WinRate = {n_m['oos']['win_rate']:.1f}%, PosDays = {n_m['oos']['positive_days_pct']:.1f}%, Trades = {n_m['oos']['trades']}"
        )
        print("3. Metrics for BANKNIFTY:")
        print(
            f"   - Train Period: ADR = {b_m['train']['adr']:.4f}%, Sharpe = {b_m['train']['sharpe']:.2f}, MaxDD = {b_m['train']['max_dd']:.2f}%, WinRate = {b_m['train']['win_rate']:.1f}%, PosDays = {b_m['train']['positive_days_pct']:.1f}%, Trades = {b_m['train']['trades']}"
        )
        print(
            f"   - Validation Period: ADR = {b_m['val']['adr']:.4f}%, Sharpe = {b_m['val']['sharpe']:.2f}, MaxDD = {b_m['val']['max_dd']:.2f}%, WinRate = {b_m['val']['win_rate']:.1f}%, PosDays = {b_m['val']['positive_days_pct']:.1f}%, Trades = {b_m['val']['trades']}"
        )
        print(
            f"   - Out-of-Sample Period: ADR = {b_m['oos']['adr']:.4f}%, Sharpe = {b_m['oos']['sharpe']:.2f}, MaxDD = {b_m['oos']['max_dd']:.2f}%, WinRate = {b_m['oos']['win_rate']:.1f}%, PosDays = {b_m['oos']['positive_days_pct']:.1f}%, Trades = {b_m['oos']['trades']}"
        )
