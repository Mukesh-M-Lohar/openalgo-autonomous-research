...
def compute_signals(df: pd.DataFrame, df_htf: Optional[pd.DataFrame] = None) -> dict:
    ...
    return {"long": bool(long_signal), "short": bool(short_signal), "indicators": indicators}
...
class SupertrendRSIPullbackBot:
    ...
    def get_htf_historical_data(self) -> pd.DataFrame:
        ...
        try:
            cache_expiry = 300
            current_time = time.time()
            if self._cache_htf_df is not None and (
                current_time - self._cache_htf_last_fetched < cache_expiry
            ):
    ...
    def check_funds(self) -> bool:
        try:
            ...
            if isinstance(r, dict) and r.get("status") == "success":
                ...
                return True
            logger.warning(
                f"Funds API did not return success. Responding dictionary: {r}. Proceeding order execution."
            )
            return True
        except Exception as e:
    ...
    def place_partial_exit_order(self, qty: int, target_label: str):
        ...
        logger.info(
            f"Placing partial exit order [{target_label}] via {exit_action} for {qty} shares..."
        )
        ...
        if resp.get("status") == "success":
            ...
            logger.info(
                f"Partial Exit Successful @ {exit_price:.2f} for {qty} shares. Reason: {target_label}"
            )
            self.notify(
                exit_action, f"PARTIAL EXIT SUCCESS ({target_label})", exit_price, f"Qty: {qty}"
            )
        else:
            logger.error(f"Partial Exit order failed: {resp}")
            self.notify(
                exit_action,
                f"PARTIAL EXIT FAILED ({target_label}): {resp.get('message', resp)}",
                0.0,
            )
    ...
    def place_full_exit_order(self, reason: str):
        ...
        if resp.get("status") == "success":
            ...
            self.notify(
                exit_action, f"FULL EXIT FAILED ({reason}): {resp.get('message', resp)}", 0.0
            )
        ...
        if self.position is not None:
            ...
            if self.position == "BUY":
                ...
                if not self.tp1_hit and current_price >= self.tp1_price:
                    logger.info(
                        f"Long TP1 reached ({current_price:.2f} >= {self.tp1_price:.2f}). Exiting {self.qty_tp1} shares."
                    )
                    self.tp1_hit = True
                ...
                # Check TP2 hit (only valid if TP1 is already hit)
                    ...
                    logger.info(
                        f"Long TP2 reached ({current_price:.2f} >= {self.tp2_price:.2f}). Exiting remaining."
                    )
                    exit_triggered = True
                    exit_reason = "TP2"
                # Check Stop Loss
                    ...
                    logger.info(
                        f"Long Stop Loss hit ({current_price:.2f} <= {sl_price:.2f}). Exiting remaining."
                    )
                    exit_triggered = True
                    exit_reason = "SL"
                ...
                if not self.tp1_hit and current_price <= self.tp1_price:
                    logger.info(
                        f"Short TP1 reached ({current_price:.2f} <= {self.tp1_price:.2f}). Exiting {self.qty_tp1} shares."
                    )
                    self.tp1_hit = True
                    self.place_partial_exit_order(self.qty_tp1, "TP1")
                    self.remaining_qty -= self.qty_tp1
                ...
                # Check TP2 hit
                    ...
                    logger.info(
                        f"Short TP2 reached ({current_price:.2f} <= {self.tp2_price:.2f}). Exiting remaining."
                    )
                    exit_triggered = True
                    exit_reason = "TP2"
                # Check Stop Loss
                    ...
                    logger.info(
                        f"Short Stop Loss hit ({current_price:.2f} >= {sl_price:.2f}). Exiting remaining."
                    )
                    exit_triggered = True
                    exit_reason = "SL"
