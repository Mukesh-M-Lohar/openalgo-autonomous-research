...
import numpy as np
import pandas as pd
from search_orb_fade import load_and_preprocess, compute_ema, simulate_orb_fade, NIFTY_PATH, BANKNIFTY_PATH
...
def compare_performance():
    ...
    print("\n--- NIFTY ---")
    print(f"Strategy: Total Return = {n_strat_ret:.2f}%, Daily Return = {n_strat_adr:.4f}%, Sharpe = {n_strat_sharpe:.2f}, Max Drawdown = {n_strat_dd:.2f}%")
    print(f"Buy & Hold: Total Return = {n_bh_ret:.2f}%, Daily Return = {n_bh_adr:.4f}%, Sharpe = {n_bh_sharpe:.2f}, Max Drawdown = {n_bh_dd:.2f}%")
    
    print("\n--- BANKNIFTY ---")
    print(f"Strategy: Total Return = {b_strat_ret:.2f}%, Daily Return = {b_strat_adr:.4f}%, Sharpe = {b_strat_sharpe:.2f}, Max Drawdown = {b_strat_dd:.2f}%")
    print(f"Buy & Hold: Total Return = {b_bh_ret:.2f}%, Daily Return = {b_bh_adr:.4f}%, Sharpe = {b_bh_sharpe:.2f}, Max Drawdown = {b_bh_dd:.2f}%")
