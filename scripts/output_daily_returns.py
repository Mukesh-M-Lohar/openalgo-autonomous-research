...
def generate_daily_report():
    ...
    daily_df["Combined_Return_Pct"] = (daily_df["NIFTY_Return_Pct"] + daily_df["BANKNIFTY_Return_Pct"]) / 2.0
    ...
    for col in daily_df.columns:
        ...
        stats[col] = {
            ...
            "Daily Win Rate (on trading days)": f"{(sum(active_series > 0) / len(active_series) * 100.0):.1f}%" if len(active_series) > 0 else "0.0%",
            "Max Daily Return": f"{series.max():.2f}%",
            "Min Daily Return": f"{series.min():.2f}%",
            "Avg Return on Trading Days": f"{active_series.mean():.4f}%" if len(active_series) > 0 else "0.0000%"
        }
