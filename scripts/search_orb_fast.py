...
def simulate_orb_fast(days_list: list, params: dict, commission_pct=0.03, slippage_pct=0.02) -> tuple:
...
def evaluate_metrics_fast(n_train, n_val, b_train, b_val, params: dict) -> dict:
    # Evaluate NIFTY
    n_train_t, n_train_r, n_train_e = simulate_orb_fast(n_train, params)
    n_val_t, n_val_r, n_val_e = simulate_orb_fast(n_val, params)
    
    # Evaluate BANKNIFTY
    b_train_t, b_train_r, b_train_e = simulate_orb_fast(b_train, params)
    b_val_t, b_val_r, b_val_e = simulate_orb_fast(b_val, params)
...
if __name__ == "__main__":
    ...
    print(f"Grid Search finished. Found {len(results)} valid configurations.")
