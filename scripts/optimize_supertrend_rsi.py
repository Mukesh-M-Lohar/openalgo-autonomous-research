...
def generate_signals(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    if params.get("use_htf", False):
        ...
        # map back to 15m
        df["htf_direction"] = df.index.map(lambda t: df_1h["htf_dir"].asof(t) if not df_1h.empty else 1)
...
# --- Simulator ---
def simulate_trades(df: pd.DataFrame, params: dict, initial_cash=100000.0, commission_pct=0.03, slippage_pct=0.02) -> tuple:
            ...
            if exit_triggered:
                ...
                if reason in ["EOD", "SL"]:
                    if not tp1_hit:
                        ...
                        pnl2 = ((exit_price - entry_price) / entry_price if position == 1 else (entry_price - exit_price) / entry_price) - cost_factor
...
if __name__ == "__main__":
    ...
    print(f"NIFTY Train ADR: {n_metrics['train']['adr']:.4f}%, Sharpe: {n_metrics['train']['sharpe']:.2f}, MaxDD: {n_metrics['train']['max_dd']:.2f}%, Trades: {n_metrics['train']['trades']}")
    print(f"NIFTY Val ADR: {n_metrics['val']['adr']:.4f}%, Sharpe: {n_metrics['val']['sharpe']:.2f}, MaxDD: {n_metrics['val']['max_dd']:.2f}%, Trades: {n_metrics['val']['trades']}")
    print(f"BANKNIFTY Train ADR: {b_metrics['train']['adr']:.4f}%, Sharpe: {b_metrics['train']['sharpe']:.2f}, MaxDD: {b_metrics['train']['max_dd']:.2f}%, Trades: {b_metrics['train']['trades']}")
    print(f"BANKNIFTY Val ADR: {b_metrics['val']['adr']:.4f}%, Sharpe: {b_metrics['val']['sharpe']:.2f}, MaxDD: {b_metrics['val']['max_dd']:.2f}%, Trades: {b_metrics['val']['trades']}")
    ...
    for idx, exp in enumerate(experiments):
        ...
        print(f"   - Train Period: ADR = {n_m['train']['adr']:.4f}%, Sharpe = {n_m['train']['sharpe']:.2f}, MaxDD = {n_m['train']['max_dd']:.2f}%, Trades = {n_m['train']['trades']}")
        print(f"   - Validation Period: ADR = {n_m['val']['adr']:.4f}%, Sharpe = {n_m['val']['sharpe']:.2f}, MaxDD = {n_m['val']['max_dd']:.2f}%, Trades = {n_m['val']['trades']}")
        print(f"   - Out-of-Sample Period: ADR = {n_m['oos']['adr']:.4f}%, Sharpe = {n_m['oos']['sharpe']:.2f}, MaxDD = {n_m['oos']['max_dd']:.2f}%, Trades = {n_m['oos']['trades']}")
        print("3. Metrics for BANKNIFTY:")
        print(f"   - Train Period: ADR = {b_m['train']['adr']:.4f}%, Sharpe = {b_m['train']['sharpe']:.2f}, MaxDD = {b_m['train']['max_dd']:.2f}%, Trades = {b_m['train']['trades']}")
        print(f"   - Validation Period: ADR = {b_m['val']['adr']:.4f}%, Sharpe = {b_m['val']['sharpe']:.2f}, MaxDD = {b_m['val']['max_dd']:.2f}%, Trades = {b_m['val']['trades']}")
        print(f"   - Out-of-Sample Period: ADR = {b_m['oos']['adr']:.4f}%, Sharpe = {b_m['oos']['sharpe']:.2f}, MaxDD = {b_m['oos']['max_dd']:.2f}%, Trades = {b_m['oos']['trades']}")
